  ...
  function _write(col, data) {
    ...
    try {
      ...
      if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED' || e.code === 22) {
        ...
        // Purge de emergência: corta mais agressivo (7 dias vendas, 3 dias auditoria)
        try {
          ...
          // Tenta salvar novamente após purge
          localStorage.setItem(_key[col], JSON.stringify(data));
          console.info('[Store] Purge de emergência concluído — dado salvo.');
        } catch(e2) {
          // Se ainda falhar, mantém só no cache (dados seguros em memória até próximo sync)
  ...
  return {
    ...
    mutateEstoque(fn) {
      _mutate('estoque', (data) => {
        fn(data);
        // Sincroniza aliases duplos após cada mutação
        // HTMLs antigos: precoUn / custoUn / qtdUn
        // estoqueService: precoVenda / precoCusto / estoqueAtual
        data.forEach(p => {
...
const SyncService = (() => {
  ...
  function push(col) {
    // Push desativado — SyncQueue (services/syncService.js) assume este papel.
    // Manter aqui causaria dupla sincronização: SyncQueue + SyncService
    // enviando a mesma coleção para o Firestore ao mesmo tempo.
    // Se SyncQueue não estiver carregado ainda, o Store._mutate cuida da fila via _pendingSync.
    if (!window.CH?.SyncQueue) {
      // Fallback extremo: só age se SyncQueue não existir (ex: página sem services carregados)
      _fila.add(col);
      clearTimeout(_timer);
  ...
  async login(pin) {
    ...
    setTimeout(() => {
      ...
      if (Number(uso.percentual) > 80) {
        // Acima de 80%: purge proativo antes de estourar
...
const CartService = (() => {
  ...
  return {
    ...
    add(prod, qtd = 1, label = 'UNID', preco = null) {
      ...
      // Custo correto: se for pack, multiplica custo unitário pela qtd do pack
      // ex: pack de 6 com custoUn=4 → custo do pack = 24, não 4
