...
(function () {
  ...
  async function _processItem(item) {
    if (item.acao === 'salvar') {
      await FirebaseService.salvar(item.colecao, item.dados);
  ...
  async function processar() {
    ...
    if (!fbOk) {
      console.info('[SyncQueue] Firebase não disponível — aguardando conexão.');
      return;
    }
  ...
  if (navigator.onLine) {
    setTimeout(() => { _drainPendingSync(); processar(); }, 3000);
  }
